...
$quotationDetails = $this->db->query("SELECT * FROM (SELECT qh.heading_id
	,qh.enquiry_id
...
WHERE ae.end_date IS NULL AND ae.user_id = $createdby  ) T WHERE row_num = 1 ORDER BY quotation_sent_date DESC")->result();
else
$quotationDetails = $this->db->query("SELECT * FROM (SELECT qh.heading_id
	,qh.enquiry_id
	,e.name AS company_name
...
WHERE ae.end_date IS NULL) T WHERE row_num = 1 ORDER BY quotation_sent_date DESC")->result();
...
WHERE ae.end_date IS NULL AND ae.user_id = $createdby) T WHERE row_num = 1 
ORDER BY quotation_sent_date DESC")->result();
...
WHERE ae.end_date IS NULL) T WHERE row_num = 1
ORDER BY quotation_sent_date DESC")->result();
    }
...
LEFT JOIN address a ON a.enquiry_id = e.enquiry_id WHERE qh.created_by = $createdby AND (
		...
		WHERE ae.end_date IS NULL AND ae.user_id = $createdby) T WHERE row_num = 1
		ORDER BY quotation_sent_date DESC
   ")->result();
        }
        else{
        $quotationDetails = $this->db->query("SELECT * FROM (SELECT qh.heading_id
