...
$purchaseTransID = $_GET['purchase_transport_id'];
   ...
   $ExistingtransportDetails = $this->db->query("SELECT * FROM purchase_transport pt INNER JOIN transport t ON t.transport_id = pt.transport_id LEFT JOIN purchase_item p ON p.purchase_item_id = pt.purchase_item_id WHERE pt.is_active = 1 and pt.purchase_transport_id = $purchaseTransID ")->result();
    foreach($ExistingtransportDetails as $getExistingtransportDetails){
        ...
        $trans_gst = $getExistingtransportDetails->gst;
        $trans_igst = $getExistingtransportDetails->total_igst;
        $trans_cgst = $getExistingtransportDetails->total_cgst;
        $trans_sgst = $getExistingtransportDetails->total_sgst;
        $trans_total_tax = $getExistingtransportDetails->total_tpt_tax;
        $received_by = $getExistingtransportDetails->Received_by;
        $trans_net_amount = $getExistingtransportDetails->netamt;
    }
   }
