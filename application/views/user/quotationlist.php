...
if(isset($_POST['search'])){
	...
	if ($from_date && $to_date && $agent_name) {
		$quotationDetails = $this->db->query ( "SELECT qh.heading_id, qh.enquiry_id, qh.company AS company_name,
       e.type AS company_type, qh.to_email AS contact_person_name, CONCAT(a.address_type, ',', a.address_line_1, ',', a.address_line_2, ',', a.country, ',', a.STATE, ',', a.city, ',', a.street, ',', a.pincode) AS address, qh.created_datetime AS quotation_sent_date,
       (SELECT name FROM user u WHERE u.user_id = qh.created_by) AS AgentName FROM `quotation_heading` qh INNER JOIN enquiry
           e ON e.enquiry_id = qh.enquiry_id INNER JOIN agent_enquiry ae ON ae.enquiry_id = e.enquiry_id LEFT JOIN address a ON a.enquiry_id = e.enquiry_id 
          WHERE ae.end_date IS NULL AND (SELECT name FROM user u WHERE u.user_id = qh.created_by) ='$agent_name' AND (qh.created_datetime BETWEEN '$from_date' AND '$to_date') ORDER BY qh.created_datetime DESC" )->result ();
	}
}
                            ...
                            <div class="card-body">
                                ...
                                <form method="post" action="<?php echo base_url()."Enquiry/quotationlist" ?>" autocomplete="off">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-4">
                                            <input class="form-control" type="text" name="agent_name" id="agent_name" list="agent_names" value="<?php echo $agent_name ?>" placeholder="Agent Name" required>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            ...
                                            foreach($quotationDetails as $getquotationDetails){ 
                                                $ToCompany = $this->db->query("SELECT * FROM enquiry WHERE isactive = 1 AND enquiry_id = $getquotationDetails->enquiry_id ")->row();
                                            $c++;
												...
												if($getquotationDetails->company_name == 'SuperEditors' || $getquotationDetails->company_name == 'Coose Company')
													$reference_no = 'SE'.date("Y",strtotime($getquotationDetails->quotation_sent_date)).'-'.$nextyear .'/'.$integer;
												else
													$reference_no = 'MW'.date("Y",strtotime($getquotationDetails->quotation_sent_date)).'-'.$nextyear .'/'.$integer;
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td>
														<a href="<?php echo base_url() ?>Enquiry/quotation?enquiry_id=<?php echo $getquotationDetails->enquiry_id ?>&header_id=<?=$getquotationDetails->heading_id?>" target="_blank">
                                                       		 <?php echo $getquotationDetails->company_name ?>
                                                        </a>
                                                    </td>
                                                        <td><?php echo $ToCompany->name?></td>
                                                      <td><?php echo $ToCompany->type?></td>
                                                    ...
                                                    <?php } ?>
                                                     ...
                                                     <td>
														...
														<a href="<?php echo base_url() ?>Enquiry/duplicate_quotation?enquiry_id=<?php echo $getquotationDetails->enquiry_id ?>&header_id=<?=$getquotationDetails->heading_id?>" target="_blank">
